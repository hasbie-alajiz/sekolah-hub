            ...
            <div class="card bg-white shadow-sm border border-gray-100 p-6">
                <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST" class="space-y-4">
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="name" value="{{ old('name', $menu->name) }}" placeholder="Contoh: Menu Utama" class="input input-bordered w-full @error('name') input-error @enderror" required />
                        @error('name')
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $menu->slug) }}" placeholder="slug-menu-otomatis" class="input input-bordered w-full text-sm" />
                        @error('slug')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
