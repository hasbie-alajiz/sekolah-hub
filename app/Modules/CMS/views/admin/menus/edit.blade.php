            ...
            <div class="card bg-white shadow-sm border border-gray-100 p-6">
                <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST" class="space-y-4">
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="name" value="{{ old('name', $menu->name) }}" placeholder="Contoh: Menu Utama" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1 @error('name') input-error @enderror" required />
                        @error('name')
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $menu->slug) }}" placeholder="slug-menu-otomatis" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1" />
                        @error('slug')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-control w-full">
                        ...
                        <select name="location" class="select select-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1">
                            ...
                            <option value="header-menu" {{ old('location', $menu->location) === 'header-menu' ? 'selected' : '' }}>Header Menu (Navigasi Atas)</option>
                            <option value="footer-menu" {{ old('location', $menu->location) === 'footer-menu' ? 'selected' : '' }}>Footer Menu (Navigasi Bawah)</option>
                        </select>
