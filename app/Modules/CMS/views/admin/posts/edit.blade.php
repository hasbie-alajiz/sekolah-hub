                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="card bg-white shadow-sm border border-gray-100 p-6">
                        <div class="form-control w-full mb-4">
                            <label class="label font-medium text-gray-700">Judul Berita</label>
                            <input type="text" name="title" value="{{ old('title', $post->title) }}" placeholder="Ketik judul berita di sini..." class="input input-bordered w-full @error('title') input-error @enderror" required />
                            @error('title')
                        ...
                        <div class="form-control w-full mb-4">
                            ...
                            <input type="text" name="slug" value="{{ old('slug', $post->slug) }}" placeholder="url-berita-otomatis-jika-kosong" class="input input-bordered w-full text-sm" />
                            @error('slug')
                                <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-control w-full mb-4">
                            <label class="label font-medium text-gray-700">Kutipan Singkat (Excerpt)</label>
                            <textarea name="excerpt" rows="3" placeholder="Ringkasan berita singkat..." class="textarea textarea-bordered w-full text-sm">{{ old('excerpt', $post->excerpt) }}</textarea>
                            @error('excerpt')
                                <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="card bg-white shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="form-control w-full mb-4">
                            ...
                            <input type="text" name="seo_title" value="{{ old('seo_title', $post->seo_title) }}" placeholder="Judul khusus untuk hasil pencarian Google" class="input input-bordered w-full text-sm" />
                        </div>

                        <div class="form-control w-full">
                            <label class="label font-medium text-gray-700">SEO Description</label>
                            <textarea name="seo_description" rows="3" placeholder="Deskripsi khusus untuk Google snippet" class="textarea textarea-bordered w-full text-sm">{{ old('seo_description', $post->seo_description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="card bg-white shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('admin.posts.index') }}" class="btn btn-ghost">Batal</a>
                        </div>
                    </div>

                    <!-- Categories Card -->
                    <div class="card bg-white shadow-sm border border-gray-100 p-6">
                        <h3 class="font-semibold text-gray-800 border-b border-gray-100 pb-3 mb-4">Kategori Berita</h3>
                        
                        <div class="form-control max-h-60 overflow-y-auto space-y-2 pr-2">
                    ...
                    <div class="card bg-white shadow-sm border border-gray-100 p-6">
                        ...
                        <button type="button" onclick="document.getElementById('media_modal').showModal()" class="btn btn-sm btn-outline btn-primary w-full">
                            Pilih Gambar
                        </button>
                    </div>
                </div>
