                ...
                <form action="{{ route('admin.ppdb.tracks.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Jalur Zonasi, Jalur Prestasi Akademik" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1 @error('name') input-error @enderror" required />
                            @error('name')
                                <span class="text-rose-600 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="number" name="quota" value="{{ old('quota') }}" min="1" placeholder="Contoh: 100" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1 @error('quota') input-error @enderror" />
                            @error('quota')
                                <span class="text-rose-600 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-control w-full">
                            ...
                            <textarea name="description" placeholder="Penjelasan singkat mengenai persyaratan atau ketentuan jalur ini..." class="textarea textarea-bordered w-full h-24 rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary focus:ring-1 @error('description') textarea-error @enderror">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="text-rose-600 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-control mt-4">
                            <label class="flex items-center gap-3 cursor-pointer justify-start">
                                ...
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} class="checkbox checkbox-primary checkbox-sm rounded" />
